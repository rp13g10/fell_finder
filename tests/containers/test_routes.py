...
from fell_finder.containers.routes import (
    StepData,
    RouteGeometry,
    RouteMetrics,
    Route,
...
class TestRoute:
    ...
    @patch("fell_finder.containers.routes.app_config")
    def test_overlap_n_nodes_long_route(self, mock_app_config: MagicMock):
        """Overlap should be set according to threshold for long routes"""
        # Arrange
        test_route = Route(0)
        test_route.points = list(range(350))

        test_config = {"routing": {"route_overlap_threshold": 0.1}}
        mock_app_config.__getitem__.side_effect = test_config.__getitem__

        target = 35

        # Act
        result = test_route.overlap_n_nodes

        # Assert
        assert result == target

    @patch("fell_finder.containers.routes.app_config")
    def test_overlap_n_nodes_short_route(self, mock_app_config: MagicMock):
        """Overlap should never go below 3"""
        # Arrange
        test_route = Route(0)
        test_route.points = list(range(15))
    ...
    @patch("fell_finder.containers.routes.app_config")
    def test_first_n_nodes(self, mock_app_config: MagicMock):
        """Overlap should be set according to threshold for long routes"""
        # Arrange
        test_route = Route(0)
        test_route.points = list(range(350))
        ...
        # Act
        result = test_route.first_n_nodes

        # Assert
        assert result == target

    @patch("fell_finder.containers.routes.app_config")
    def test_last_3_nodes(self, mock_app_config: MagicMock):
        """Overlap should be set according to threshold for long routes"""
        # Arrange
        test_route = Route(0)
